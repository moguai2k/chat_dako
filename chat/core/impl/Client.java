...
public class Client implements ChatEventListener {
    ...
    //TODO: Auskommentierter Code by Chris in separater Klasse abspeichern, da sehr nice und aufw�ndig war <3
/*	public int ContainsCount(String SearchPhrase, String SearchText) {
		...
		if ((this.smileys) && containsSmileys(text)) {// wenn Smileys aktiviert AND iein Smiley vorkommt
			...
			// Bild ersetzen
				for (int i = 1; i < 7; i++) {
					...
					case 1:
						smiley = "=\\)"; // ":)" nicht m�glich sonst regex.PatternSyntaxException, daher DoppelSlash
						break;
					case 2:
						smiley = ";\\)";
						break;
    ...
    public void onMessage(String username, String message, String time) {
		...
		//EditorKit setzen und Smiley einf�gen///////
